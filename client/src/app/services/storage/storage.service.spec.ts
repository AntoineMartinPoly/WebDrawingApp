...
import { DISABLE, ERROR, SESSION_STORAGE, TUTORIAL } from 'src/constant/storage/constant';
...
describe('StorageService', () => {
  ...
  it('set should add the a value linked with a name', () => {
    const service: StorageService = TestBed.get(StorageService);
    service.set(TUTORIAL, DISABLE);
    expect(localStorage.getItem(TUTORIAL)).toMatch(DISABLE);
  });

  it('add should add the a value to sessionStorage if specified', () => {
    const service: StorageService = TestBed.get(StorageService);
    service.set(TUTORIAL, DISABLE, SESSION_STORAGE);
  ...
  it('deleteAll should delete all values in sessionStorage if specified', () => {
    const service: StorageService = TestBed.get(StorageService);
    service.set(TUTORIAL, DISABLE, SESSION_STORAGE);
    service.set(DISABLE, TUTORIAL, SESSION_STORAGE);
    expect(sessionStorage.getItem(TUTORIAL)).toMatch(DISABLE);
